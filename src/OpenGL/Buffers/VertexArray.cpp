...
void VertexArray::linkBuffers() {
    ...
    m_vbo->bind();
    if (m_isEBOused) m_ebo->bind();

    int offset = 0;
    for (int i = 0; i < m_vbo->m_vertexAttribCount; i++) {
        glVertexAttribPointer(
            i, m_vbo->m_AttribsLayouts[i].count, m_vbo->m_AttribsLayouts[i].type, m_vbo->m_AttribsLayouts[i].normalized,
            m_vbo->m_stride, (const void*)(std::size_t)(offset)
        );
...
void VertexArray::addVertexBuffer(VertexBuffer* vbo_ptr) { m_vbo = vbo_ptr; }
